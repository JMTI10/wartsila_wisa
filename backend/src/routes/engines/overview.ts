...
router.get('/:engineId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const efficiencyResult = await operationsDb.query(
      `SELECT 
    ...
    // Calculate metrics
    const heatRate =
      efficiencyData.total_fuel_energy_mwh &&
    ...
    // Get yearly data
    const yearlyResult = await operationsDb.query(
      `SELECT 
    ...
    const dailyEmissionsResult = await operationsDb.query(
      `SELECT 
       ...
       ORDER BY eg.measurement_date DESC`,
      [engineId]
    );

    const dailyEmissions = dailyEmissionsResult.rows.map((row: any) => ({
      date: row.date,
      ...
      co2_intensity:
        row.daily_net_generation_mwh > 0
      ...
      data: {
        ...
        efficiency_metrics: {
          ...
          total_operating_hours: parseFloat(
            efficiencyData.total_operating_hours || 0
          ),
          average_capacity_factor: parseFloat(
            efficiencyData.avg_capacity_factor || 0
          ),
          total_net_generation: parseFloat(
            efficiencyData.total_net_generation_mwh || 0
          ),
        ...
        yearly_data: yearlyResult.rows.map((row: any) => ({
