// services/engines/dailyEmissions.ts
...
export async function getEngineDailyEmissions(
  operationsDb: OperationsDatabase,
  engineId: string
): Promise<{ success: boolean; data?: DailyEmissionData[]; message?: string }> {
  try {
    ...
    const dailyEmissions: DailyEmissionData[] = dailyEmissionsResult.rows.map(
      (row: any) => {
        ...
        return {
          ...
          co2_intensity_calculated:
            dailyGeneration > 0 ? co2Emissions / dailyGeneration : 0,
          co2_intensity_unit: 'kg/MWh',
          hourly_co2_rate: parseFloat(row.hourly_co2_rate || 0),
          hourly_co2_rate_unit: 'kg/hour',
          daily_operating_hours_calculated: parseFloat(
            row.daily_operating_hours || 0
          ),
