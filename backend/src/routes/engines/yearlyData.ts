// services/engines/yearlyData.ts
...
export async function getEngineYearlyData(
  operationsDb: OperationsDatabase,
  engineId: string
): Promise<{ success: boolean; data?: YearlyData[]; message?: string }> {
  try {
    // Get yearly data
    ...
    const yearlyData: YearlyData[] = yearlyResult.rows.map((row: any) => ({
      ...
      net_generation_annual_calculated: parseFloat(
        row.net_generation_annual || 0
      ),
      net_generation_annual_unit: 'MWh',
      operating_hours_annual_calculated: parseFloat(
        row.operating_hours_annual || 0
      ),
      capacity_factor_annual_calculated: parseFloat(
        row.capacity_factor_annual || 0
      ),
    }));
