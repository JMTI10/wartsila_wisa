// services/engines/efficiencyMetrics.ts
...
export async function getEngineEfficiencyMetrics(
  operationsDb: OperationsDatabase,
  engineId: string
): Promise<{ success: boolean; data?: EfficiencyMetrics; message?: string }> {
  try {
...
export async function getTotalNetGeneration(
  operationsDb: OperationsDatabase,
  engineIds?: string[]
): Promise<{
  success: boolean;
  ...
  try {
    ...
    const totalNetGeneration = parseFloat(
      result.rows[0].total_net_generation_mwh || 0
    );
