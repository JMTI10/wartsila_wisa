// services/engines/emissionsMetrics.ts
...
export async function getEngineEmissionsMetrics(
  operationsDb: OperationsDatabase,
  engineId: string,
  totalNetGeneration: number,
  totalOperatingHours: number
): Promise<{ success: boolean; data?: EmissionsMetrics; message?: string }> {
  try {
    // Get emission rates (average from ALL available data, not just recent)
    const emissionRatesResult = await operationsDb.query(
      `SELECT 
        pollutant_type,
    ...
    const emissionRates = emissionRatesResult.rows.reduce(
      (acc: any, row: any) => {
    ...
    const co2Intensity =
      emissionRates.co2_rate && totalNetGeneration && totalOperatingHours
        ? (emissionRates.co2_rate * totalOperatingHours) / totalNetGeneration
        : null;

    // Get total net generation from ALL connected engines
    const totalNetGenerationAllEnginesResult = await operationsDb.query(
      `SELECT SUM(eg.net_generation) as total_net_generation_all_engines
       FROM engine_generation eg`
    );

    const totalNetGenerationAllEngines = parseFloat(
      totalNetGenerationAllEnginesResult.rows[0]
        ?.total_net_generation_all_engines || 0
    );

    // Get total revenue from ALL connected engines
    const totalRevenueAllEnginesResult = await operationsDb.query(
      `SELECT SUM(eg.revenue) as total_revenue_all_engines
       FROM engine_generation eg
       WHERE eg.revenue IS NOT NULL`
    );

    const totalRevenueAllEngines = parseFloat(
      totalRevenueAllEnginesResult.rows[0]?.total_revenue_all_engines || 0
    );

    // Get total annual fuel consumption from ALL connected engines
    const totalFuelConsumptionResult = await operationsDb.query(
      `SELECT SUM(efc.annual_consumption) as total_fuel_consumption_all_engines
       FROM engine_fuel_consumption efc
       WHERE efc.annual_consumption IS NOT NULL`
    );

    const totalFuelConsumptionAllEngines = parseFloat(
      totalFuelConsumptionResult.rows[0]?.total_fuel_consumption_all_engines ||
        0
    );

    // Get annual emissions totals for all engines for emissions per unit production calculation
    const annualEmissionsResult = await operationsDb.query(
      `SELECT 
        pollutant_type,
    ...
    const annualEmissions = annualEmissionsResult.rows.reduce(
      (acc: any, row: any) => {
        acc[`${row.pollutant_type}_annual`] = parseFloat(
          row.total_annual_emissions
        );
        return acc;
      },
      {}
