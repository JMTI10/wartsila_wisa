// services/plants/plantCarbonEmissionsService.ts
...
export interface CarbonEmissionsCalculated {
  ...
  co2_intensity_calculated: number; // kg CO2 per MWh
  co2_equivalent_intensity_calculated: number; // kg CO2 equivalent per MWh
  total_net_generation_mwh?: number; // Net energy generation for transparency
  total_gross_generation_mwh?: number; // Gross energy generation for transparency
  measurement_period: string;
}

export class PlantCarbonEmissionsService {
  /**
   ...
   */
  static async getCarbonEmissions(
    operationsDb: OperationsDatabase,
    businessPlantId: string
  ): Promise<CarbonEmissionsCalculated> {
    try {
      ...
      const carbonEmissionsCalculated: CarbonEmissionsCalculated = {
      ...
      if (carbonEmissionsResult.rows.length > 0) {
        ...
        const latestDate = carbonEmissionsResult.rows[0].latest_date
          ?.toISOString()
          .split('T')[0];
        carbonEmissionsCalculated.measurement_period = `${
          earliestDate || 'N/A'
        } to ${latestDate || 'N/A'}`;
      }
      ...
      carbonEmissionsResult.rows.forEach((row) => {
        ...
        (carbonEmissionsCalculated as any)[`total_${pollutant}_all_time`] =
          totalAllTime;
      ...
      const totalNetGeneration = parseFloat(
        energyGenerationResult.rows[0]?.total_net_generation_mwh || 0
      );
      const totalGrossGeneration = parseFloat(
        energyGenerationResult.rows[0]?.total_gross_generation_mwh || 0
      );
      ...
      if (totalNetGeneration > 0) {
        ...
        console.warn(
          `⚠️ No net generation data found for plant ${businessPlantId}, intensity calculations set to 0`
        );
      }
