// services/plants/plantOperatingSummaryService.ts
...
export class PlantOperatingSummaryService {
  /**
   ...
   */
  static async getOperatingSummary(
    operationsDb: OperationsDatabase,
    businessPlantId: string
  ): Promise<OperatingSummaryCalculated> {
    try {
      ...
      const costPerMwhProducedCalculated =
        totalNetGeneration > 0 ? totalAllTimeFuelCost / totalNetGeneration : 0;
      ...
      const operatingSummaryCalculated: OperatingSummaryCalculated = {
        total_all_time_fuel_cost: totalAllTimeFuelCost,
        cost_per_mwh_produced_calculated:
          Math.round(costPerMwhProducedCalculated * 100) / 100, // Round to 2 decimals
