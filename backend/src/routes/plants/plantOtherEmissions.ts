// services/plants/plantOtherEmissionsService.ts
...
export interface OtherEmissionsCalculated {
  ...
  nox_intensity_calculated?: number; // kg NOx per MWh
  sox_intensity_calculated?: number; // kg SOx per MWh
  pm10_intensity_calculated?: number; // kg PM10 per MWh
  pm25_intensity_calculated?: number; // kg PM2.5 per MWh
  measurement_period: string;
  [key: string]: number | string | undefined; // For dynamic pollutant properties
}

export class PlantOtherEmissionsService {
  /**
  ...
  static async getOtherEmissions(
    ...
    try {
      ...
      const otherEmissionsCalculated: OtherEmissionsCalculated = {
      ...
      otherEmissionsResult.rows.forEach((row) => {
        ...
        if (totalNetGeneration && totalNetGeneration > 0) {
          ...
          otherEmissionsCalculated[`${pollutantType}_intensity_calculated`] =
            Math.round(
              ((totalAllTime * intensityMultiplier) / totalNetGeneration) * 100
            ) / 100;
        }
      });
