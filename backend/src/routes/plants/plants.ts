...
router.get('/:plantId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const plantData = plantResult.rows[0];
    ...
    const enginesSummary = {
      ...
      decommissioned_count: engines.filter((e) => e.status === 'decommissioned')
        .length,
      total_nameplate_capacity: engines.reduce(
    ...
    const energyTotals = {
      ...
      average_capacity_factor: parseFloat(
      ...
      measurement_period: `${
        ...
        energyResult.rows[0]?.latest_date?.toISOString().split('T')[0] || 'N/A'
      }`,
    };
    ...
    if (carbonEmissionsResult.rows.length > 0) {
      ...
      const latestDate = carbonEmissionsResult.rows[0].latest_date
        ?.toISOString()
        .split('T')[0];
      carbonEmissions.measurement_period = `${earliestDate || 'N/A'} to ${
        latestDate || 'N/A'
      }`;
    } else {
    ...
    carbonEmissionsResult.rows.forEach((row) => {
      ...
      co2EquivalentTotal +=
        totalAllTime * gwpFactors[pollutant as keyof typeof gwpFactors];
    });
    ...
    if (otherStackEmissionsResult.rows.length > 0) {
      ...
      particulateEmissionsResult.rows.length > 0 &&
    ...
    otherStackEmissionsResult.rows.forEach((row) => {
      otherEmissions[`total_${row.pollutant_type}_all_time`] = parseFloat(
        row.total_all_time || 0
      );
    });
    ...
    if (pmData) {
      otherEmissions.total_pm10_all_time = parseFloat(
        pmData.total_pm10_all_time || 0
      );
      otherEmissions.total_pm25_all_time = parseFloat(
        pmData.total_pm25_all_time || 0
      );
    }
    ...
    const totalAllTimeFuelCost = fuelCostsResult.rows.reduce(
      (sum, row) => sum + parseFloat(row.total_all_time_fuel_cost || 0),
      0
    );

    const costPerMwhProduced =
      energyTotals.total_net_generation > 0
    ...
    if (allowanceResult.rows.length > 0) {
      ...
      const allowancesSurplus =
        allowanceData.total_allowances_available - allowancesNeeded;

      allowances = {
        allocation_year: parseInt(allowanceData.allocation_year),
        total_allowances_available: parseInt(
          allowanceData.total_allowances_available
        ),
