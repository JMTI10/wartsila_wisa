// services/plants/plantRenewableService.ts
import OperationsDatabase from '../../database/connections/operationsDb';

export interface RenewableFuelsCalculated {
  renewable_fuel_percentage: number; // % of total fuel consumption from renewable sources
  carbon_fuel_percentage: number; // % of total fuel consumption from carbon-based sources
  renewable_production_percentage: number; // % of total energy production from renewable fuels
  carbon_production_percentage: number; // % of total energy production from carbon fuels
  total_renewable_consumption: number; // Total renewable fuel consumption
  total_carbon_consumption: number; // Total carbon fuel consumption
  total_fuel_consumption: number; // Total fuel consumption across all types
  total_renewable_production: number; // Total energy production from renewable fuels (MWh)
  total_carbon_production: number; // Total energy production from carbon fuels (MWh)
  total_energy_production: number; // Total energy production (MWh)
...
export class PlantRenewableService {
  /**
   ...
   */
  static async getRenewableFuels(
    operationsDb: OperationsDatabase,
    businessPlantId: string
  ): Promise<RenewableFuelsCalculated> {
    try {
      ...
      const renewableFuelsCalculated: RenewableFuelsCalculated = {
      ...
      if (fuelConsumptionResult.rows.length > 0) {
        ...
        const latestDate = fuelConsumptionResult.rows[0].latest_date
          ?.toISOString()
          .split('T')[0];
        renewableFuelsCalculated.measurement_period = `${
          earliestDate || 'N/A'
        } to ${latestDate || 'N/A'}`;
      }
      ...
      fuelConsumptionResult.rows.forEach((row) => {
        ...
        aggregated.total_net_generation += parseFloat(
          row.total_net_generation || 0
        );
      ...
      Array.from(fuelAggregation.values()).forEach((fuel) => {
        ...
        const isRenewable = renewableFuelTypes.some(
          (renewableType) =>
            fuelType?.includes(renewableType) ||
            fuelName?.toLowerCase().includes(renewableType)
        );

        // Check if fuel is carbon-based
        const isCarbon = carbonFuelTypes.some(
          (carbonType) =>
            fuelType?.includes(carbonType) ||
        ...
        if (isRenewable) {
          ...
          console.warn(
            `⚠️ Unclassified fuel type: ${fuelType} (${fuelName}) - defaulting to carbon fuel`
          );
          totalCarbonConsumption += consumption;
          totalCarbonProduction += production;
          renewableFuelsCalculated.carbon_fuels_breakdown.push({
